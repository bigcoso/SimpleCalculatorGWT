...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "calculatorgwt" because\n';
  ...
  $wnd.__gwt_activeModules['calculatorgwt'] = {
    'moduleName' : 'calculatorgwt',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/calculatorgwt.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['calculatorgwt'].superdevmode = true;

  var expectedSuffix = '/calculatorgwt.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + calculatorgwt + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:calculatorgwt';
  var devModeSessionKey = '__gwtDevModeSession:calculatorgwt';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/calculatorgwt/calculatorgwt.nocache.js';
  ...
  $wnd.__gwt_activeModules['calculatorgwt'].canRedirect = true;
  $wnd.__gwt_activeModules['calculatorgwt'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING calculatorgwt';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/calculatorgwt?user.agent=' + ua + '&_callback=' + callback;
